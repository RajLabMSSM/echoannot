...
#' 
#' #### piggyback ####                  
#' tmp <- file.path(tempdir(),"CORCES_2020.bulkATACseq_peaks.tsv.gz")
#' data.table::fwrite(CORCES_2020.bulkATACseq_peaks,tmp,sep="\t")
#' piggyback::pb_upload(file = tmp,
#'                      repo = "RajLabMSSM/echoannot")
#' } 
#' @export
get_CORCES_2020.bulkATACseq_peaks <- function(){
    tmp <- get_data(fname = "CORCES_2020.bulkATACseq_peaks.tsv.gz")
