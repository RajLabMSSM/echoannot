...
#' \dontrun{
#' BST1 <- echodata::BST1
#' dat <- ROADMAP.tabix(chrom=BST1$CHR[1],
...
ROADMAP.tabix <- function(results_path=tempfile(
    fileext = "ROADMAP_query.csv.gz"),
                          chrom,
                          min_pos,
                          max_pos,
                          eid,
    ...
    dat <- echotabix::query_tabular(fullSS_tabix = URL, 
                                    chrom = chrom, 
                                    start_pos = min_pos, 
                                    end_pos = max_pos, 
                                    local = FALSE)
    ...
    if(convert_to_granges){
        dat <- dt_to_granges(subset_DT = dat, 
                             chrom_col = "Chrom",
                             start_col = "Start",
                             end_col = "End", 
                             style = "NCBI") 
