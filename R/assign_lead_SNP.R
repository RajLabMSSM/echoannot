...
#'
#' If none of the SNPs in the data.frame have \code{leadSNP==TRUE},
...
#' @return Fine-mapping results data.frame with
#' new boolean \strong{leadSNP} column,
#'  indicating whether each SNPs is the lead GWAS SNP in that locus or not.
#' @keywords internal
#' @importFrom dplyr arrange desc
#' @importFrom utils head
assign_lead_SNP <- function(new_DT,
                            verbose = TRUE) {
    P <- Effect <- NULL

    if (sum(new_DT$leadSNP, na.rm = TRUE) == 0) {
        messager("+ leadSNP missing. Assigning new one by min p-value.",
            v = verbose
        )
        top.snp <- utils::head(dplyr::arrange(
            new_DT, P,
            dplyr::desc(Effect)
        ))[1, ]$SNP
        new_DT$leadSNP <- ifelse(new_DT$SNP == top.snp, TRUE, FALSE)
    }
    return(new_DT)
}
