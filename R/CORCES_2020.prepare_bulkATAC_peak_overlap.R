...
CORCES_2020.prepare_bulkATAC_peak_overlap <- function(merged_DT,
    ...
    if (annotate_genes) {
        ...
        messager("CORCES_2020:: Annotating peaks by bulk brain target genes",
            v = verbose
        )
        peak_overlap <- subset(
        ...
        for (column in annot_cols) {
            dict <- stats::setNames(peak_overlap[[column]], peak_overlap$Peak_ID)
    ...
    if (return_counts) {
        ...
        if (sum(dat_melt$Count == 0 | is.na(dat_melt$Count), na.rm = TRUE) > 0) {
            try({
