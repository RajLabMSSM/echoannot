...
```{r import_data, eval=has_internet}
...
*Note:* This chunk is kept as `eval=FALSE` because `annotate_snps` queries
external databases (HaploReg, RegulomeDB, BioMart), which can be slow and
...
```{r epigenomics, eval=has_internet}
...
```{r super_summary, eval=has_internet}
super_plot <- echoannot::super_summary_plot(merged_DT = merged_DT,
                                            plot_missense = FALSE)

```

```{r Cleanup, include=FALSE, eval=has_internet}
