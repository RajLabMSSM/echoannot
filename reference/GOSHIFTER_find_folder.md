...
Locate the directory containing the `goshifter.py` script. By default,
looks for the bundled copy shipped with echolocatoR at
`inst/tools/goshifter/`. A user-supplied path takes precedence.

## Usage

``` r
GOSHIFTER_find_folder(goshifter = NULL)
```
...
- goshifter:
  ...
  Path to the GoShifter directory. If `NULL`, the bundled copy in
  echolocatoR is used.

## Value

Path to the GoShifter directory.
